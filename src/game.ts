...
ER.Game = class Game {
  ...
  _update(dt) {
    ...
    if (this.ambient.thunderStruck) this.audio.playThunder();
    ...
    if (this.state === 'playing') {
      ...
      if (this.player.dryness <= 0) this._triggerGameOver();
    }
  ...
  _updatePlaying(dt) {
    ...
    if (!wasJumping && this.player.isJumping) this.audio.playJump();
