...
ER.RainSystem = class RainSystem {
    ...
    resize(w, h) {
        ...
        for (const key in this.layers)
            this.layers[key].init(w, h, this.rng);
    }
            ...
            for (let i = 0; i < L.count; i++) {
                ...
                if (L.y[i] > this.h + 20) {
                    ...
                    if (opt === this.layers.mid.opts || opt === this.layers.fg.opts) {
                        if (this.rng() < 0.5)
                            this.spawnSplash(L.x[i], this.h - 4 + this.rng() * 4, dir.x, 1, 0.8);
                    }
                }
                if (L.x[i] < -220)
                    L.x[i] += this.w + 400;
    ...
    renderLayer(ctx, key, color) {
        ...
        const L = this.layers[key];
        const opt = L.opts;
        const dx = (_a = this.dirX) !== null && _a !== void 0 ? _a : 0, dy = (_b = this.dirY) !== null && _b !== void 0 ? _b : 1;
        ctx.save();
