'use strict';
/* ============================================================================
 * AS THE DROPLETS FALL — collision.js
...
ER.CollisionManager = class CollisionManager {
    ...
    resolve(player, rain, px, py) {
        ...
        for (const key of ['mid', 'fg']) {
            ...
            for (let i = 0; i < L.count; i++) {
                ...
                if (x < px - this.checkRadius || x > px + this.checkRadius)
                    continue;
                if (y < bodyMinY - 40 || y > bodyMaxY)
                    continue;
                if (player.umbrella.shields(x - pivotX, y - pivotY)) {
                    L.hit[i] = 1;
                    continue;
                }
        ...
        for (const p of rain.splashes) {
            if (!p.active || p.scored)
                continue;
            if (p.x < px - this.checkRadius || p.x > px + this.checkRadius)
                continue;
            if (p.y < bodyMinY - 20 || p.y > bodyMaxY + 10)
                continue;
            if (player.umbrella.shields(p.x - pivotX, p.y - pivotY)) {
                p.scored = true;
                continue;
            }
    ...
    resolveCarSplash(player, px, py, splashSx, splashSy) {
        ...
        if (dist > 95)
            return;
        const pivotX = px, pivotY = py - this.pivotUp;
        if (player.umbrella.shields(splashSx - pivotX, splashSy - pivotY))
            return;
        player.addWetness(4);
    }
};
